...
baseDir = os.path.abspath(os.path.dirname(__file__))#must test
...
class Root():
	...
	# UpLoad image
	@cherrypy.expose
	def upload(self, myFile,nameImg,authorImg,): #self =this in java
		h = hashlib.sha256()

		filename = baseDir + "/uploads/" + myFile.filename #creating file dir in server folder uploads
		...
		# nameImg and authorImg are input parameters of this method <-------------------------------imp
	...
	def list(self, id):
		...
		if (id == "all"):
			...
			result = db.execute("SELECT * FROM images WHERE author LIKE")
